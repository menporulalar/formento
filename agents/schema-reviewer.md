...
If `delta` is present (migration path — not expected during Milestone 1–4 fresh-rebuild work per $CLAUDE_PLUGIN_ROOT/docs/decisions/0003-fresh-rebuild-default-defer-migration.md and $CLAUDE_PLUGIN_ROOT/docs/decisions/0004-derived-mode-extraction-first.md, but check anyway in case it appears): confirm every `DeltaChange` with `breaking: true` has a `resolution`before this review can recommend approval. This mirrors the checkpoint gate's own enforcement (PRD R2) — you're an independent check on it, not a replacement for it.
