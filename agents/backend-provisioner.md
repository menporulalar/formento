...
- **`techStackPreference` names PHP + MySQL** (e.g. `{"backend": "PHP", "database": "MySQL"}`, as set when a derived-mode project's user explicitly chose to keep their original stack): generate a plain PHP + MySQL backend instead — see "PHP + MySQL target" below. Do not silently fall back to the Node/Prisma default; that would override an explicit, already-confirmed user decision (see `$CLAUDE_PLUGIN_ROOT/docs/decisions/0003-fresh-rebuild-default-defer-migration.md` / project-level stack consent noted in the Spec IR).
- **Any other named stack:** this agent doesn't yet have a built-in profile for it — stop and surface the gap to the user/orchestrating session rather than guessing at unfamiliar tooling conventions, the same way `extraction-agent` treats an unhandled source shape (`$CLAUDE_PLUGIN_ROOT/docs/decisions/0007-milestone-1-source-confirmed.md`).
...
- **Known MySQL/MariaDB DDL portability gotcha:** "MySQL" locally often actually means **MariaDB** — XAMPP, WAMP, and Laragon (the most common local PHP dev stacks on Windows) all bundle MariaDB by default, not real MySQL, even though the client binary is still called `mysql`/`mysql.exe`. The two engines diverge on some DDL syntax: MySQL 8.0.16+ added `ALTER TABLE ... DROP CHECK constraint_name` specifically for dropping a CHECK constraint, but MariaDB never implemented that keyword — only the standard `DROP CONSTRAINT constraint_name` form, which MySQL 8.0.19+ also accepts as a synonym. Prefer the portable form (`DROP CONSTRAINT`, not `DROP CHECK`) so migrations run unmodified on both engines, and when a project's own dev/test environment is unknown, don't assume real MySQL just because the client is named `mysql`. Inline `CONSTRAINT ... CHECK (...)` clauses inside `CREATE TABLE` are fine on both engines (MySQL 8.0.16+, MariaDB 10.2.1+) — this gotcha is specifically about the `ALTER TABLE ... DROP` clause.
