...
Invoke the `backend-provisioner` subagent with the approved Phase 1 spec (and Phase 2 UI for field-level wiring hints) to generate the backend for the project's confirmed tech stack (Node/Prisma by default, or another stack — e.g. PHP+MySQL — when `project.techStackPreference` says so; see $CLAUDE_PLUGIN_ROOT/docs/decisions/0008-backend-provisioner-generalized-beyond-prisma.md): migrations, API routes, auth/roles, and seed/migration data, in a sandbox/local env.
