...
At Phase 0b (derived mode only): only the fresh-rebuild outcome is implemented ($CLAUDE_PLUGIN_ROOT/docs/decisions/0003-fresh-rebuild-default-defer-migration.md, $CLAUDE_PLUGIN_ROOT/docs/decisions/0004-derived-mode-extraction-first.md — migration is deferred to Milestone 5). Confirm the user wants fresh-rebuild explicitly; if they ask for the migration path, tell them it isn't built yet and point to $CLAUDE_PLUGIN_ROOT/docs/decisions/0003-fresh-rebuild-default-defer-migration.md rather than attempting it.
